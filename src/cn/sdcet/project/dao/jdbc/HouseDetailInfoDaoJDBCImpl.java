...
public class HouseDetailInfoDaoJDBCImpl implements HouseDetailInfoDao {
	...
	public HouseDetailInfo getHouseInfoDetail(int houseId) {
		...
		String sql="SELECT u.LOGINID,h.AREA,h.PRICE,h.HOUSETYPE,h.name as HOUSENAME,h.FINEFLAG,h.H_ADDRESS as H_ADDRESS,hd.HOUSEID," +
		...
		if(map.size()>0){
			...
			agent.setGender(map.get("GENDER").toString());
			agent.setHead(map.get("HEAD").toString());
			agent.setId(Integer.valueOf(map.get("AGENTID").toString()));

	...
	<div id="page">
		<div id="fh5co-blog">
			...
			ShopCarService shopcarService=new ShopCarService();
				...
				houseList = shopcarService.getShopCar(userid,0);
				}
				if(houseList.size()>0){
...
<%} }%>
			...
			List<House> houseListed = new ArrayList<House>();
				if(id==null){
					response.sendRedirect("../LoginOut/login.jsp?returnUrl="+request.getRequestURI());
				}else{
			ShopCarService shopcarService=new ShopCarService();
				//HttpSession session=new HttpSession();
				 userid=Integer.parseInt( session.getAttribute("userid").toString());
				 houseListed = shopcarService.getShopCar(userid,1);
				}
				if(houseList.size()>0){
				for(House house:houseList){
			%>
					...
					<div class="col-md-4">
						<div class="fh5co-blog animate-box">
							...
							<div class="title text-center">
								...
								<a href="../delthing?action=u&houseid=<%=house.getId()%>" onclick="return confirm('真的要取消预约吗？')">
									<span class="category" style="font-weight:lighter;">售后</span>
								</a>
							</div>
						</div>
					</div>
